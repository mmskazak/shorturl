...
var (
	...
	// ErrKeyAlreadyExists указывает на ошибку, когда ключ (например, уникальный идентификатор) уже существует в базе данных.
	...
	// Эта ошибка возникает, если в базу данных пытаются вставить дублирующие данные, что нарушает ограничения уникальности.
	ErrUniqueViolation = errors.New("error database unique violation")

	// ErrShortURLsForUserNotFound указывает на ошибку, когда короткие URL-адреса для указанного пользователя не найдены.
	// Эта ошибка может возникать при попытке получить короткие URL-адреса для пользователя, у которого нет сохраненных URL.
